...
async def process_super_ai_turn(session: CardGameSession):
    ...
    if opp_id and session.room_id in HUMAN_DRAFT_MEMORIES:
        ...
        HUMAN_DRAFT_MEMORIES[session.room_id] = [c.get("id") for c in human_deck if "id" in c]

    # 1. ドラフトフェーズ
    if session.status == "DRAFT":
        opts = session.draft_options.get(bot_id, [])
        if opts:
            my_deck_ids = [c.get("id") for c in session.decks.get(bot_id, [])]
    ...
    if session.status == "BATTLE":
        max_steps = 15  # 無限ループ防止用安全ガード
        step_count = 0

        while session.turn_user_id == bot_id and session.status == "BATTLE" and step_count < max_steps:
            step_count += 1
            action = evaluate_future_and_find_action(session, bot_id)
            if not action:
                break
...
# ====================================================
# 最強思考・未来予測・エラーガード評価エンジン
# ====================================================
def evaluate_future_and_find_action(session: CardGameSession, bot_id: str) -> Optional[dict]:
    try:
        opp_id = next((uid for uid in session.player_order if uid != bot_id), None)
        if not opp_id:
            return None
        ...
        bot_mp = session.mp.get(bot_id, 0)
        next_opp_mp = min(10, session.max_mp.get(opp_id, 1) + 1)
