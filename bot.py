...
class SuperBotEngine:
    ...
    def predict_opponent_max_damage(self, opp_id: str) -> Tuple[int, int]:
        ...
        safety_buffer = unknown_count * 3  # 未知の手札1枚につき3点警戒
        ...
        try:
            ...
            for attacker in bot_board:
                if attacker.get("can_attack") and attacker.get("frozen_turns", 0) == 0 and attacker.get("attacks_left", 0) > 0:
                    ...
                    if attacker.get("card_id") == "u_06":  # 小人の精密反撃不能攻撃
                        if opp_taunts:
            ...
            if playable:
                ...
                for card in playable:
                    ...
                    if c_type == "unit":
                        ...
                        if card.get("id") == "s_09":
                            if taunt_units:
                                return {"action": "PLAY_HAND", "card_instance_id": card["instance_id"], "target": {"type": "unit", "id": taunt_units[0]["instance_id"]}}
                            continue
